...
* `descriptors_thermostability.csv` - csv of pre-calculated descriptors for the thermostability dataset, computed using the descriptors module and `protpy` package [[2]](#references) with protpy < 1.3.0 (15 descriptors). The dimensions for the csv file are 261 x 9714 (261 protein sequences with 9714 features), using default metaparameters. This file will need to be regenerated using `get_all_descriptors()` to include all 33 descriptors available in protpy >= 1.3.0 (expected shape: 261 x 10572). The columns and dimensions of the original descriptor file are outlined below:
