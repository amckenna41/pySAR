...
* `descriptors_thermostability.csv` - pre-calculated protein descriptors using sequences from the thermostability dataset, computed with protpy v1.4.1 (33 descriptors). The dimensions for this csv are 272 x 10358 (272 protein sequences and 10358 features). This file includes all 33 descriptors available in protpy >= 1.4.1 and can be used directly without regeneration.
* `descriptors_absorption.csv` - pre-calculated protein descriptors using sequences from the absorption dataset, computed with protpy < 1.3.0 (15 descriptors). The dimensions for this csv are 81 x 9714 (81 protein sequences and 9714 features). Regenerate with `get_all_descriptors()` for the full 33-descriptor set (expected shape: 81 x 10572).
* `descriptors_enantioselectivity.csv` - pre-calculated protein descriptors using sequences from the enantioselectivity dataset, computed with protpy < 1.3.0 (15 descriptors). The dimensions for this csv are 152 x 9714 (152 protein sequences and 9714 features). Regenerate with `get_all_descriptors()` for the full 33-descriptor set (expected shape: 152 x 10572).
* `descriptors_localization.csv` - pre-calculated protein descriptors using sequences from the localization dataset, computed with protpy < 1.3.0 (15 descriptors). The dimensions for this csv are 254 x 9714 (254 protein sequences and 9714 features). Regenerate with `get_all_descriptors()` for the full 33-descriptor set (expected shape: 254 x 10572).

Each of the pre-calculated descriptor CSVs have 9714 total features (15 descriptors with default parameters, generated with protpy < 1.3.0). The columns and dimensions are outlined below:
