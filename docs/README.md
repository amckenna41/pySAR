...
The documentation for `pySAR` is hosted on the **readthedocs** platform and is available [here](https://pysar.readthedocs.io/en/latest). The documentation is automatically built and published on the platform via the repo. 
...
Create a /docs directory inside the `pysar` package
```bash
mkdir /docs
```
