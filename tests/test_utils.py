...
class UtilsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.makedirs(self.test_output_folder + "_" + _globals.CURRENT_DATETIME )
...
#1.)
        self.assertIsNotNone(utils.valid_sequence(invalid_seqs), 
                "Valid sequence function should not return None.")
...
#4.)
        ...
        self.assertIsNone(utils.valid_sequence(self.test_dataset2['sequence'][random_seq1]),
                "Valid sequence function should return None")
        self.assertIsNone(utils.valid_sequence(self.test_dataset3['sequence'][random_seq2]),
                "Valid sequence function should return None")
        self.assertIsNone(utils.valid_sequence(self.test_dataset4['sequence'][random_seq3]),
                "Valid sequence function should return None")
...
#1.)
        ...
        self.assertEqual(len(seq1_test), 1,
                "Expected length of output to be 1, got {}.".format(len(seq1_test)))
        self.assertEqual(len(seq1_test[0]), 4,
                "Expected length of output to be 4, got {}.".format(len(seq1_test[0])))
        self.assertIsInstance(seq1_test, list,
                "Expected output to be of type list, got {}.".format(type(seq1_test)))
        self.assertNotIn('-', seq1_test,
                "There shouldn't be any gaps (-) in the sequence.")
#2.)
        seq2_test = utils.remove_gaps(seq2)
        self.assertEqual(len(seq2_test), 1,
                "Expected length of output to be 1, got {}.".format(len(seq2_test)))
        self.assertEqual(len(seq2_test[0]), 6,
                "Expected length of output to be 6, got {}.".format(len(seq2_test)))
        self.assertIsInstance(seq2_test, list,
                "Expected output to be of type list, got {}.".format(type(seq2_test)))
        self.assertNotIn('-', seq2_test,
                "There shouldn't be any gaps (-) in the sequence.")
#3.)
        seq3_test = utils.remove_gaps(seq3)
        self.assertEqual(len(seq3_test), 10,
                "Expected length of output to be 10, got {}.".format(len(seq3_test)))
        self.assertIsInstance(seq3_test, str,
                "Expected output to be of type str, got {}.".format(len(seq3_test)))
        self.assertNotIn('-', seq3_test,
                "There shouldn't be any gaps (-) in the sequence.")
#4.)
        seq4_test = utils.remove_gaps(seq4)
        self.assertEqual(len(seq4_test), 13,
                "Expected length of output to be 13, got {}.".format(len(seq4_test)))
        self.assertIsInstance(seq4_test, str,
                "Expected output to be of type str, got {}.".format(len(seq4_test)))
...
#1.)
        ...
        self.assertEqual(flattened_array.shape, (6,1),
                "Expected output shape to be (6,1), got {}.".format(flattened_array.shape))
        self.assertIsInstance(flattened_array, np.ndarray,
                "Expected output to be of type np.ndarray, got {}.".format(type(flattened_array)))
        self.assertEqual(flattened_array.ndim, 2,
                "Expected 2 output dimensions, got {}.".format(flattened_array.ndim))
...
#2.)
        ...
        self.assertEqual(flattened_array_2.shape, (9,1),
                "Expected output shape to be (9,1), got {}.".format(flattened_array_2.shape))
        self.assertIsInstance(flattened_array_2, np.ndarray,
                "Expected output to be of type np.ndarray, got {}.".format(type(flattened_array_2)))
        self.assertEqual(flattened_array_2.ndim, 2,
                "Expected 2 output dimensions, got {}.".format(flattened_array_2.ndim))
...
#3.)
        ...
        self.assertEqual(flattened_array_3.shape, (40,1),
                "Expected output shape to be (40,1), got {}.".format(flattened_array_3.shape))
        self.assertIsInstance(flattened_array_3, np.ndarray,
                "Expected output to be of type np.ndarray, got {}.".format(type(flattened_array_3)))
        self.assertEqual(flattened_array_3.ndim, 2,
                "Expected 2 output dimensions, got {}.".format(flattened_array_3.ndim))
#4.)
        flattened_array_4 = utils.flatten(seq4)
        self.assertEqual(len(flattened_array_4), 6,
                "Expected length of output to be 6, got {}.".format(len(flattened_array_4)))
        self.assertIsInstance(flattened_array_4, list,
                "Expected output to be of type list, got {}.".format(type(flattened_array_4)))
        self.assertEqual(flattened_array_4, seq4,
                "Output doesn't match expected sequence {}".format(seq4))
#5.)
        flattened_array_5 = utils.flatten(seq5)
        self.assertEqual(flattened_array_5, seq5,
                "Output doesn't match expected sequence {}".format(seq5))
        self.assertIsInstance(flattened_array_5, str,
                "Expected output to be of type string, got {}.".format(type(flattened_array_5)))
...
#2.)
        ...
        self.assertEqual(len(padded_seqs1), 2,
                "Expected length of output to be 2, got {}.".format(len(padded_seqs1)))
        self.assertEqual(len(padded_seqs1[0]), 5,
                "Expected length of output to be 5, got {}.".format(len(padded_seqs1[0])))
        self.assertIsInstance(padded_seqs1, np.ndarray,
                "Expected output to be of type numpy array, got {}.".format(type(padded_seqs1)))
#3.)
        padded_seqs2 = utils.zero_padding(seq2)
        self.assertEqual(len(padded_seqs2), 3,
                "Expected length of output to be 3, got {}.".format(len(padded_seqs2)))
        self.assertEqual(len(padded_seqs2[0]), 4,
                "Expected length of output to be 4, got {}.".format(len(padded_seqs2[0])))
        self.assertIsInstance(padded_seqs2, list,
                "Expected output to be of type list, got {}.".format(type(padded_seqs2)))
#4.)
        padded_seqs3 = utils.zero_padding(seq3)
        self.assertEqual(len(padded_seqs3), 4,
                "Expected length of output to be 4, got {}.".format(len(padded_seqs3)))
        self.assertEqual(padded_seqs3.shape, (4,5,2),
                "Expected output to be of shape (4,5,2), got {}.".format(padded_seqs3.shape))
        self.assertIsInstance(padded_seqs3, np.ndarray,
                "Expected output to be of type numpy array, got {}.".format(type(padded_seqs3)))
        self.assertTrue(padded_seqs3.any() == seq3.any(),
                "Expected original and padded sequences to have the same values.")
...
#1.)
        ...
        utils.save_results(test_results, 'test_results', output_folder=self.test_output_folder)
        self.assertTrue(os.path.isfile(os.path.join(self.test_output_folder + "_" + _globals.CURRENT_DATETIME, 'test_results.csv')),
                "Output results csv not found in output folder: {}.".format(os.path.join(self.test_output_folder + "_" + _globals.CURRENT_DATETIME, 'test_results.csv')))
#2.)
        test_results1 = {'MAE': 2.10, 'MSE': 0.99, 'RPD': 1.28}
        utils.save_results(test_results1, 'test_results1', output_folder=self.test_output_folder)
        self.assertTrue(os.path.isfile(os.path.join(self.test_output_folder + "_" + _globals.CURRENT_DATETIME, 'test_results1.csv')),
...
#3.)
        ...
        utils.save_results(test_results2, 'test_results2', output_folder=self.test_output_folder)
        self.assertTrue(os.path.isfile(os.path.join(self.test_output_folder + "_" + _globals.CURRENT_DATETIME, 'test_results2.csv')),
                "Output results csv not found in output folder: {}.".format(os.path.join(self.test_output_folder + "_" + _globals.CURRENT_DATETIME, 'test_results2.csv')))
#4.)
        test_results3 = pd.Series(np.random.randint(1,100), index=['Col1', 'Col2', 'Col3', 'Col4'])
        utils.save_results(test_results3, 'test_results3', output_folder=self.test_output_folder)
        self.assertTrue(os.path.isfile(os.path.join(self.test_output_folder + "_" + _globals.CURRENT_DATETIME, 'test_results3.csv')),
                "Output results csv not found in output folder: {}.".format(os.path.join(self.test_output_folder + "_" + _globals.CURRENT_DATETIME, 'test_results3.csv')))
#5.)
        ...
        self.assertFalse(os.path.isfile(os.path.join(self.test_output_folder + "_" + _globals.CURRENT_DATETIME, 'test_results4.csv')),
                "")
...
#1.)
        ...
        self.assertIsInstance(test_map1, dict, 
                "Expected instance to be of type dict, got {}.".format(type(test_map1)))
        self.assertEqual(test_map1.first_name, "Joe", 
                "Expected Joe, got {}.".format(test_map1.first_name))
        self.assertEqual(test_map1.last_name, "Bloggs", 
                "Expected Bloggs, got {}.".format(test_map1.last_name))
        self.assertEqual(test_map1.country, "Ireland", 
                "Expected Ireland, got {}.".format(test_map1.country))
        self.assertEqual(test_map1.city, "Dublin", 
                "Expected Dublin, got {}.".format(test_map1.city))
        self.assertEqual(len(test_map1), 4, 
                "Expected output length to be 4, got {}.".format(len(test_map1)))
#2.)
        test_map2 = utils.Map({"first_name":"John", "last_name":"Smith"}, country="Germany", city="Hanover")

        self.assertIsInstance(test_map2, dict, 
                "Expected instance to be of type dict, got {}.".format(type(test_map2)))
        self.assertEqual(test_map2.first_name, "John", 
                "Expected John, got {}.".format(test_map2.first_name))
        self.assertEqual(test_map2.last_name, "Smith", 
                "Expected Smith, got {}.".format(test_map2.last_name))
        self.assertEqual(test_map2.country, "Germany", 
                "Expected Germany, got {}.".format(test_map2.country))
        self.assertEqual(test_map2.city, "Hanover", 
                "Expected Hanover, got {}.".format(test_map2.city))
        self.assertEqual(len(test_map2), 4, 
                "Expected output length to be 4, got {}.".format(len(test_map2)))
#3.)
        test_map3 = utils.Map({})

        self.assertIsInstance(test_map3, dict, 
                "Expected instance to be of type dict, got {}.".format(type(test_map3)))
        self.assertEqual(test_map3, {}, 
                "Expected an empty dict, got {}.".format(test_map3))
        self.assertEqual(len(test_map3), 0, 
                "Expected output length to be 0, got {}.".format(len(test_map3)))
#4.)    
        test_map1.language = "Python"
        test_map1["age"] = 42
        self.assertEqual(test_map1.language, "Python", 
                "Expected Python, got {}.".format(test_map1.language))
        self.assertEqual(test_map1.age, 42, 
                "Expected 42, got {}.".format(test_map1.age))
        self.assertEqual(len(test_map1), 6, 
                "Expected output length to be 6, got {}.".format(len(test_map1)))

        test_map2.language = "C++"
        test_map2.age = 20
        self.assertEqual(test_map2.language, "C++", 
                "Expected C++, got {}.".format(test_map2.language))
        self.assertEqual(test_map2.age, 20, 
                "Expected 20, got {}.".format(test_map2.age))
        self.assertEqual(len(test_map2), 6, 
                "Expected output length to be 6, got {}.".format(len(test_map2)))

        test_map3.language = "Ruby"
        test_map3.age = 99
        self.assertEqual(test_map3.language, "Ruby", 
                "Expected Ruby, got {}.".format(test_map3.language))
        self.assertEqual(test_map3.age, 99, 
                "Expected 99, got {}.".format(test_map3.age))
        self.assertEqual(len(test_map3), 2, 
                "Expected output length to be 2, got {}.".format(len(test_map3)))
#5.)
        del test_map1.first_name
        self.assertEqual(len(test_map1), 5, 
                "Expected output length to be 5, got {}.".format(len(test_map1)))
        del test_map1.country
        self.assertEqual(len(test_map1), 4, 
                "Expected output length to be 4, got {}.".format(len(test_map1)))
        del test_map3.language
        self.assertEqual(len(test_map3), 1, 
